...
app.get('/external-top-developers', async (req, res) => {
  try {
    const response = await fetch('http://localhost:3003/top-developers');
    if (!response.ok) throw new Error('Failed to fetch from nft-top-developer');
    const data = await response.json();
    res.json(data);
  } catch (error) {
    console.error('Error fetching external top developers:', error.message);
...
app.get('/pending-count', authenticate, requireAdmin, async (req, res) => {
  try {
